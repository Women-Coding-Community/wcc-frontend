...
1. Run quality checks — execute and capture output:

   ```bash
   pnpm lint:fix && pnpm format && pnpm type-check
   ```

   Report errors/warnings. Flag `type-check` failures as `[CRITICAL]`; unfixable lint/format violations as `[WARNING]`.
...
3. Review with a code-review mindset — same priority order as PR review:
...
4. Check conventions and quality for changed technologies only:
...
5. Output the review in two parts:

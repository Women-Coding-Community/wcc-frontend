...
Report any errors or warnings produced. If `type-check` fails, flag every type error as `[CRITICAL]`. If lint or format produces unfixable violations, flag them as `[WARNING]`.

## Step 2 — Gather local diff
...
- Test names follow Given-When-Then descriptive `it()`/`test()` (JS)
